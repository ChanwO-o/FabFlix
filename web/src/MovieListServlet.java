		...
		if (fulltext != null && !fulltext.isEmpty()) {
			...
			try {
				...
				while (rs.next()) {
					...
					String titletemp = rs.getString("entry");
					//jsonObject.addProperty("movie_title", titletemp);
					// Get a connection from dataSource
					Connection dbcon2 = dataSource.getConnection();
					System.out.println("CHECK 2 = " + titletemp);
					String query2 = "select movies.id,movies.title,movies.year,movies.director,ratings.rating,group_concat(stars.id) as star_id" +
							", substring_index(group_concat(distinct genres.name separator ','), ',', 3) as genres, " +
							"group_concat(stars.name) as stars from movies left join genres_in_movies on movies.id=genres_in_movies.movieId" +
							" left join ratings on ratings.movieId=movies.id left join genres on " +
							"genres.id=genres_in_movies.genreId left join stars_in_movies on movies.id=stars_in_movies.movieId " +
							"left join stars on stars_in_movies.starId=stars.id where movies.title = ?";

					// Declare our PreparedStatement
					PreparedStatement statement2 = dbcon2.prepareStatement(query2);
					// apply params
					statement2.setString(1, titletemp);
					...
					while (rs2.next()) {
						...
						String star_id = rs2.getString("star_id");
						...
						String movie_genres = rs2.getString("genres");
						String movie_stars = rs2.getString("stars");
					...
					System.out.println("CHECK 3 = " + jsonArray.toString());
